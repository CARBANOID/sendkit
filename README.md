...
- create a basic `cli` using commander : [index.ts](/packages/cli/src/index.ts)
  ...
  program
    ...
    .action(async (chatId: string, message: string) => {
      // argument (chatId: string, message: string) are extracted in order of their defination
...
- we can't publish the packages as it is cause it is uses an enviroment variable (`TELEGRAM_BOT_TOKEN`)

- go to `package/cli` and run

  ```
  bun add zod
  ```

- go to [index.ts](packages/cli/src/index.ts) and add these utilities
...
- for publishing the packages , we need to format and lint the code

- we are using `oxc` : [https://oxc.rs/](https://oxc.rs/)

- packages to install :

  ```sh
      bun add -d oxfmt oxlint tsdown typescript @types/node
  ```

- now create **configuration file** for the formater : [`.oxfmtrc.json`](.oxfmtrc.json)

- now create **configuration file** for the linter : [`.oxlintrc.json`](.oxlintrc.json)

- add these `scripts` in the root [`package.json`](package.json) file

  ```json
  {
    "scripts": {
      "format": "oxfmt . --write", // to format the whole project
      "format:check": "oxfmt . --check", // to check if the whole project is formatted or not
      "lint": "oxlint . --deny-warnings", // to lint the whole project
      "lint:fix": "oxlint . --fix" // to fix the linting issues in the whole project
    }
  }
  ```

* add one more `script` in the root [`package.json`](package.json)

  ```json
  {
    "scripts": {
      "typecheck": "tsc --noEmit" // verify our code againt the tsconfig.json
    }
  }
  ```

* remove **"@types/node"** from "devDependencies" in every `package.json` file expect the root `package.json` file .
